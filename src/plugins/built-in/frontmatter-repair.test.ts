...
import { FrontmatterRepair, frontmatterEnhancer, frontmatterValidator } from './frontmatter-repair.js';
...
Content here.`;
      ...
      const result = repair.validateContent(content, 'test.md');
      expect(result.valid).toBe(false);
      expect(result.issues.some(i => i.message.includes('Missing required frontmatter field: title'))).toBe(true);
    });
