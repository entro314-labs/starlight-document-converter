...
export class ContentAnalyzer {
   ...
   */
  private detectContentType(content: string, filePath: string): 'guide' | 'reference' | 'tutorial' | 'blog' | 'documentation' {
    const contentLower = content.toLowerCase();
    const pathLower = filePath.toLowerCase();
    ...
    const tutorialScore = tutorialIndicators.reduce((score, indicator) => 
      score + (contentLower.includes(indicator) ? 1 : 0), 0);
    const referenceScore = referenceIndicators.reduce((score, indicator) => 
      score + (contentLower.includes(indicator) ? 1 : 0), 0);
    const guideScore = guideIndicators.reduce((score, indicator) => 
      score + (contentLower.includes(indicator) ? 1 : 0), 0);
  ...
  private extractTopics(content: string, filePath: string): string[] {
    ...
    const pathParts = filePath.split('/').map(part => 
      part.replace(/[-_]/g, ' ').replace(/\.[^.]+$/, '')
    );
