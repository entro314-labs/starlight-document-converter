...
export const markdownProcessor: FileProcessor = {
  ...
  metadata: {
    ...
    version: '1.0.0',
    description: 'Advanced markdown processing with link fixing, image processing, and TOC generation',
  ...
  process: async (content: string, context: ProcessingContext): Promise<string> => {
    ...
    try {
      ...
      if (context.options.fixLinks || context.options.processImages || context.options.generateToc) {
