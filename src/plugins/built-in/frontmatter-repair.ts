...
export const frontmatterValidator: QualityValidator = {
  ...
  validate: (content: string, metadata: DocumentMetadata, context: ProcessingContext) => {
    ...
    return {
      ...
      level:
        validation.score?.overall === 'good'
          ? 'high'
          : validation.score?.overall === 'fair'
            ? 'medium'
            : 'low',
...
export class FrontmatterRepair {
  ...
  repairFrontmatter(content: string, filePath: string): RepairResult {
    ...
    try {
      ...
      if (!metadata.description) {
        metadata.description = this.generateDescriptionFromContent(
          contentBody,
          metadata.title || ''
        )
      ...
      if (metadata.description) {
        ...
        if (
          cleanDesc !== metadata.description ||
          metadata.description.length > this.maxDescriptionLength
        ) {
          metadata.description =
            cleanDesc.length > this.maxDescriptionLength
  ...
  private calculateQualityScore(
    ...
    if (metadata?.title) {
      titleScore =
        metadata.title.length > 10 && metadata.title.length <= this.maxTitleLength ? 100 : 60
    }
