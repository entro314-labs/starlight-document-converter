...
export const markdownEnhancer: MetadataEnhancer = {
  ...
  enhance: async (metadata: DocumentMetadata, context: ProcessingContext): Promise<DocumentMetadata> => {
    ...
    try {
      ...
      const { metadata: analyzedMetadata, analysis } = analyzer.analyzeContent(content, context.inputPath);
      ...
      const enhancedMetadata: DocumentMetadata = {
        ...
        tags: [...(metadata.tags || []), ...(analyzedMetadata.tags || [])].filter((tag, index, arr) => arr.indexOf(tag) === index), // Remove duplicates
