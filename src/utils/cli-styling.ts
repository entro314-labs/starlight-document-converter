...
// Box styling presets
export const boxes = {
  success: (text: string, title?: string) => boxen(text, {
    padding: 1,
    margin: 1,
    borderStyle: 'round',
    borderColor: 'green',
  ...
  error: (text: string, title?: string) => boxen(text, {
    padding: 1,
    margin: 1,
    borderStyle: 'round',
    borderColor: 'red',
  ...
  info: (text: string, title?: string) => boxen(text, {
    padding: 1,
    margin: 1,
    borderStyle: 'round',
    borderColor: 'cyan',
  ...
  warning: (text: string, title?: string) => boxen(text, {
    padding: 1,
    margin: 1,
    borderStyle: 'round',
    borderColor: 'yellow',
...
export const createTable = (options?: any) => new Table({
  style: { head: ['cyan'] },
  chars: {
    'top': '─',
    'top-mid': '┬',
...
export const createIssuesTable = (issues: Array<{ type: string; message: string; severity?: number; file?: string }>) => {
  const table = createTable({
    head: ['File', 'Type', 'Issue', 'Severity']
  });

  issues.forEach(issue => {
    const severityIcon = issue.severity && issue.severity >= 7 ? status.error('High') :
                        issue.severity && issue.severity >= 4 ? status.warning('Med') :
                        status.info('Low');
    
    const typeIcon = issue.type === 'error' ? colors.error(issue.type) :
                    issue.type === 'warning' ? colors.warning(issue.type) :
                    colors.info(issue.type);
    
    table.push([
      issue.file ? colors.dim(issue.file) : '-',
      typeIcon,
      issue.message,
      severityIcon
    ]);
...
export const formatHelpSection = (title: string, items: Array<{ name: string; description: string }>) => {
  const lines = [
    '',
