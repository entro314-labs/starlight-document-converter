...
This document has proper frontmatter.`;
  ...
  describe('Package Dependencies', () => {
    ...
    it('should not have broken existing dependencies', async () => {
      ...
      const { stdout } = await execAsync('node -e "const pkg = require(\'./dist/index.js\'); console.log(typeof pkg.DocumentConverter)"');
  ...
  describe('Performance and Memory', () => {
    ...
    it('should not leak memory with styling utilities', async () => {
      ...
      const promises = Array.from({ length: 10 }, () => 
        execAsync('node dist/cli.js --version')
      );
  ...
  describe('Backward Compatibility', () => {
    ...
    it('should support all existing CLI commands', async () => {
      ...
      for (const command of commands) {
        const { stdout } = await execAsync(`node dist/cli.js ${command} --help`);
