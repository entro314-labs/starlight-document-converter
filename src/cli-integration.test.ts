...
import { createBrandHeader, formatHelpSection, createResultsTable, createIssuesTable } from './utils/cli-styling.js';
...
describe('CLI Integration Tests', () => {
  ...
  describe('Brand Header Integration', () => {
    it('should create consistent brand headers across different contexts', () => {
      ...
      const longHeader = createBrandHeader('Very Long Application Name That Tests Wrapping Behavior');
  ...
  describe('Help Section Integration', () => {
    it('should format complex help sections consistently', () => {
      const basicCommands = [
        ...
        { name: 'batch <input>', description: 'Convert multiple files or entire directories efficiently' },
        { name: 'setup', description: 'Interactive project setup wizard for new Astro Starlight projects' }
      ];

      const advancedCommands = [
        { name: 'repair <input>', description: 'Fix frontmatter, links, images, and content structural issues' },
        { name: 'validate <input>', description: 'Validate content structure, quality scoring, and compliance checks' },
        { name: 'watch <input>', description: 'Monitor directory for changes and automatically convert new files' }
      ];
    ...
    it('should handle edge cases in help formatting', () => {
      const edgeCases = [
        ...
        { name: 'very-long-command-name-that-might-wrap', description: 'Very long description that might wrap to multiple lines and should be handled gracefully by the formatting system' },
        { name: 'special-chars', description: 'Description with "quotes", <brackets>, and & symbols' }
      ];
